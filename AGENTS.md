...
**Global Ask Shortcut**: Control+X is captured via a listen-only `CGEvent` tap (rather than an AppKit global monitor) so it's detected reliably while the app is running in the background. The tap is listen-only, so it never swallows the keystroke — Control+X still works normally elsewhere (and the macOS cut shortcut is Command+X, not Control+X, so there's no cut conflict).
...
| `CompanionPanelView.swift` | ~745 | SwiftUI panel content for the menu bar dropdown. Shows companion status, the Control+X instructions, model picker (Haiku/Sonnet), arrow + cursor size/color pickers, permissions UI, and quit button. Dark aesthetic using `DS` design system. |
...
| `GlobalAskShortcutMonitor.swift` | ~165 | System-wide Control+X shortcut monitor. Owns the listen-only `CGEvent` tap and publishes press/release transitions. |
